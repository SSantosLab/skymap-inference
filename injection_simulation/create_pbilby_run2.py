...
prior = """{
  ...
  psi = bilby.core.prior.Uniform(name='psi', minimum=0, maximum=np.pi, boundary='periodic'),
  phase = bilby.core.prior.Uniform(name='phase', minimum=0, maximum=2 * np.pi, boundary='periodic'),
  ra = bilby.gw.prior.HealPixPrior(bilby.gw.prior.HealPixMapPriorDist('data/catalog_skymap.fits'), names=['ra', 'dec'], bounds={'ra': (0, 2*np.pi), 'dec': (-np.pi/2, np.pi/2)}), 'ra'),
  dec = bilby.gw.prior.HealPixPrior(bilby.gw.prior.HealPixMapPriorDist('data/catalog_skymap.fits'), names=['ra', 'dec'], bounds={'ra': (0, 2*np.pi), 'dec': (-np.pi/2, np.pi/2)}), 'dec')
}
"""
